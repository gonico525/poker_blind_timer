...
Level 9:  800/1600, Ante 200
Level 10: 1200/2400, Ante 300
```
- **上昇率**: 約1.5-2倍（緩やかに上昇）
- **想定スタック**: 5000-10000スタート
- **想定時間**: 約2-3時間（レベル時間15分）

#### ✅ ターボトーナメント
**速いペースで決着（10レベル）**
```
Level 1:  50/100,    Ante 0
Level 2:  100/200,   Ante 25
Level 3:  200/400,   Ante 50
Level 4:  300/600,   Ante 100
Level 5:  500/1000,  Ante 100
Level 6:  800/1600,  Ante 200
Level 7:  1200/2400, Ante 300
Level 8:  2000/4000, Ante 500
Level 9:  3000/6000, Ante 1000
Level 10: 5000/10000, Ante 1500
```
- **上昇率**: 約2倍（急速上昇）
- **想定スタック**: 5000-10000スタート
- **想定時間**: 約1.5-2時間（レベル時間10分）

#### ✅ ディープスタックトーナメント
**じっくりプレイ向け（10レベル）**
```
Level 1:  25/50,    Ante 0
Level 2:  50/75,    Ante 0
Level 3:  50/100,   Ante 0
Level 4:  75/150,   Ante 25
Level 5:  100/200,  Ante 25
Level 6:  150/300,  Ante 50
Level 7:  200/400,  Ante 50
Level 8:  300/600,  Ante 100
Level 9:  400/800,  Ante 100
Level 10: 600/1200, Ante 150
```
- **上昇率**: 約1.3-1.5倍（最も緩やか）
- **想定スタック**: 10000-20000スタート
- **想定時間**: 約3-4時間（レベル時間20分）
...
- ✅ 残り1分（確定）- 重要な警告タイミング
- ✅ 残り30秒（確定）- 最終警告
- ❌ 残り5分、3分、10秒 - 頻繁すぎるため不採用

**休憩終了前:**
- ✅ レベルと同じタイミング（残り1分、30秒）
- 音声は同じものを使用（シンプルに保つ）
...
**音源選定:**
- フリー素材サイトから取得（freesound.org等）
- または Web Audio API で生成
...
**将来的拡張（優先度：中）:**
- 段階調整（0-100%）
- 個別調整（レベル変更音 / 警告音 を別々に）

### 8.3 休憩設定のデフォルト値

#### ✅ デフォルトの休憩設定
**初期状態:**
- ✅ 休憩機能: **無効**（ホームゲームでは通常休憩を挟まないため）
- ユーザーが必要に応じて有効化可能

**休憩を有効化する場合のデフォルト値:**
- 休憩頻度: 4レベルごと
  - 理由: カジノトーナメントの一般的な設定（Level 4, 8, 12...で休憩）
- 休憩時間: 10分
  - 理由: ホームゲームでは長すぎない適度な休憩時間

**設定の柔軟性:**
- ユーザーは自由に頻度（1-10レベルごと）と時間（5-30分）を変更可能

### 8.4 レベル時間のデフォルト値

#### ✅ デフォルトのレベル時間
**プリセットごとに異なるデフォルト時間を設定:**
- ✅ スタンダードトーナメント: **15分**
  - バランスの取れた時間（じっくり考えられる）
  - 想定総プレイ時間: 約2.5時間（10レベル）

- ✅ ターボトーナメント: **10分**
  - 速いペースで決着
  - 想定総プレイ時間: 約1.5-2時間（10レベル）

- ✅ ディープスタックトーナメント: **20分**
  - じっくりプレイできる
  - 想定総プレイ時間: 約3-4時間（10レベル）

**カスタマイズ:**
- ユーザーは各プリセットのレベル時間を自由に変更可能（5-60分の範囲）

### 8.5 フルスクリーンAPIの対応範囲

#### ✅ フォールバック戦略
**Fullscreen API非対応ブラウザ:**
- ✅ ボタンをグレーアウト表示（無効化）
- ツールチップで「お使いのブラウザはフルスクリーンをサポートしていません」と表示
- 通常の最大化表示を推奨するメッセージを設定画面に表示

**推奨:**
- モダンブラウザでは基本的に対応しているため、大きな問題にはならない見込み
...
**上限設定:**
- ✅ 最大50個まで保存可能
  - 理由: 実用上十分な数、localStorage容量を圧迫しない

**上限に達した場合:**
- エラーメッセージを表示: 「プリセットの上限（50個）に達しました。新しいプリセットを保存するには、不要なプリセットを削除してください。」
- 保存を拒否（既存データを保護）

**容量エラー時:**
- localStorageの容量超過エラー時は、エラーメッセージを表示し、エクスポート機能を案内
...
1. **JSON形式チェック:**
   - 不正なJSON → エラーメッセージ: 「無効なファイル形式です。正しいJSON形式のファイルを選択してください。」

2. **スキーマ検証:**
   - 必須フィールド不足 → エラーメッセージ: 「インポートファイルに必要なデータが含まれていません。」
   - バージョン不一致 → 可能なら自動変換、不可能ならエラー

3. **ブラインド値検証:**
   - SB/BB が負の数 → エラー: 「ブラインド値は正の数である必要があります。」
   - SB が 0 → エラー: 「スモールブラインドは0より大きい値である必要があります。」
   - BB が SB より小さい → 警告: 「通常、ビッグブラインドはスモールブラインドの2倍です。このまま続行しますか？」
   - Ante が負の数 → エラー: 「アンティは0以上である必要があります。」

4. **リカバリ戦略:**
   - 部分的に有効なデータの場合、有効な部分のみインポートするか確認
   - 全て無効な場合はインポート拒否
...
**精度要件:**
- ✅ 1秒あたり±50ms以内の誤差
- 10分（600秒）で最大±500ms程度の累積誤差

**実装方式:**
- ✅ **ドリフト補正付きsetInterval**（初期バージョン）
  - Date.now()で実時間を記録
  - setIntervalのドリフトを定期的に補正
  - シンプルで十分な精度

**バックグラウンドタブ対応:**
- ✅ アクティブ化時に実時間で補正
  - タブが非アクティブになった時刻を記録
  - アクティブ化時に経過時間を再計算して同期

**将来的拡張（優先度：低）:**
- Web Workers を使用した高精度タイマー（必要に応じて）

### 8.9 キーボードショートカットの競合

#### ✅ 設定画面でのキーボードショートカット
**入力フィールドでの挙動:**
- ✅ 入力フィールド（input, textarea）にフォーカスがある時は、タイマー操作系ショートカットを無効化
  - 無効化対象: スペース、R、左右矢印キー
  - 常に有効: Esc、F（フルスクリーン）、?（ヘルプ）

**Escキーの優先順位:**
1. モーダル/ダイアログが開いている → モーダルを閉じる
2. モーダルがなくフルスクリーン中 → フルスクリーン解除
3. どちらでもない → 何もしない

**実装:**
- イベントリスナーで `event.target.tagName` をチェック
- モーダルの有無を状態管理
...
**永続化:**
- ユーザーが手動で切り替えた場合は、その選択をlocalStorageに保存
- 次回起動時は保存された設定を優先

**フォールバック:**
- `prefers-color-scheme`非対応ブラウザ → ダークモードをデフォルト
  - 理由: ポーカータイマーは暗い環境での使用が多いため
...
**アニメーションON/OFF設定:**
- ✅ 初期バージョンでは実装しない（シンプルに保つ）
- ✅ アクセシビリティ対応として、`prefers-reduced-motion`に対応
  - このメディアクエリが有効な場合、アニメーションを無効化またはフェードのみに簡略化

**将来的拡張（優先度：低）:**
- ユーザーが手動でアニメーションON/OFFを切り替える設定

### 8.12 ブラウザ対応範囲

#### ✅ 最小サポートバージョン
**対象ブラウザ:**
- ✅ Chrome: **v90以降**（2021年4月リリース）
- ✅ Firefox: **v88以降**（2021年4月リリース）
- ✅ Safari: **v14以降**（2020年9月リリース）
- ✅ Edge: **v90以降**（2021年4月リリース、Chromiumベース）

**理由:**
- これらのバージョンで以下の機能がネイティブサポートされる:
  - ES2020構文（Optional Chaining, Nullish Coalescing等）
  - CSS Grid, Flexbox
  - localStorage
  - Web Audio API
  - Fullscreen API
...
**ポリフィル:**
- ✅ 基本的に不要（モダンブラウザの機能のみ使用）
- TypeScriptのトランスパイル設定: ES2020ターゲット

**ブラウザチェック:**
- 非対応ブラウザでアクセスした場合、警告メッセージを表示
- 「モダンブラウザの最新版をご利用ください」
...
以下が揃えば機能仕様書の作成を開始可能：
- [x] 確認事項（🔴重要）への回答 ✅
- [x] 確認事項（🟡中程度）への回答（または「お任せ」の指示） ✅
- [ ] 技術スタック（React + TS + Vite）の最終承認 ⏳ ユーザー承認待ち
...
| 1.1 | 2026-01-24 | セクション8のすべての確認事項に対する検討結果を記載 | AI System Architect |
