...
- **通常時（PC）**: NextLevelInfo と**横並び**で同一行に配置
- **通常時（モバイル）**: NextLevelInfo の下に縦スタック
- **ブレイク時**: TimerControls の直上に全幅で配置（NextLevelInfo 不在のため）
- **配置の一貫性**: 常に TimerControls の直上の情報バー行内に表示される

**配置の根拠:**

1. Timer → BlindInfo はタイマーのコア情報群であり、分断すべきでない
2. NextLevelInfo と AverageStackDisplay は補助的な統計情報として同格であり、PC では同一行にまとめることで**縦方向の追加消費をゼロ**にする
3. PC（特に HD 1280×720 のノートPC）でタイマー表示の視認性（120px フォント）を最大限維持する
4. 横方向のスペース（PC では 1280px+）を有効活用する
...
- **高さ**: NextLevelInfo と揃える（横並び時にバーの高さが一致するよう `align-items: stretch`）
...
**PC（768px+）: NextLevelInfo と横並び**

```
┌──────────────────────────────┬───────────────────────────────────┐
│ Next  200 / 400  Ante: 50   │ エントリー [10] 残り [8] [-1]     │
│ 3 levels until break        │ Avg Stack  37,500 (62.5BB)        │
└──────────────────────────────┴───────────────────────────────────┘
→ 縦方向の追加消費: 0px（NextLevelInfo と同一行を共有）
```
...
```
┌─────────────────────────────────────┐
│ Next  200 / 400   3 levels til break│
├─────────────────────────────────────┤
│  エントリー [10]   残り [8]  [−1]   │
│       Avg Stack: 37,500 (62.5BB)    │
└─────────────────────────────────────┘
→ モバイルはスクロール前提のため縦方向の追加を許容
```
...
└─────────────────────────────────────────────────────────────────┘
→ NextLevelInfo が不在のため AverageStackDisplay が全幅を使用
```
...
- `initialStack` が 0（ストラクチャーに初期スタック未設定）の場合:
  ...
  - NextLevelInfo が情報バー行の全幅を使用する（従来通りのレイアウト）
...
- 通常時・ブレイク時ともに AverageStackDisplay は同じコンポーネントインスタンスで、同じ配置位置（TimerControls の直上の情報バー行内）に表示される
- BreakDisplay との疎結合を維持し、コンポーネントの再利用性を確保する
- ブレイク中もプレイヤー数の編集は操作可能（ブレイク中にプレイヤー数を確認・調整する運用を想定）
- ブレイク時は NextLevelInfo が不在のため、AverageStackDisplay が情報バー行の全幅を使用する
