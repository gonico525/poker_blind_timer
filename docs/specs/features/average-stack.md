...
- **通常時**: BlindInfo と NextLevelInfo の間、または NextLevelInfo の下に配置
- **ブレイク時**: BreakDisplay 内または付近に配置
...
- `initialStack` が 0（ストラクチャーに初期スタック未設定）の場合:
  - プレイヤー数入力欄とアベレージスタック表示を非表示にする
- `totalPlayers` が 0（参加人数未設定）の場合:
  - プレイヤー数入力欄は表示する
  - アベレージスタック値は非表示（または「---」等のプレースホルダ表示）

#### レスポンシブ対応

- モバイル: 縦並びレイアウト
- タブレット以上: 横並びレイアウト（プレイヤー数入力 | アベレージスタック表示）

### 5.3 ブレイク中の表示

BreakDisplay にもアベレージスタック情報を表示する。ブレイク中はプレイヤー数の確認・調整を行うことが多いため、プレイヤー数入力も操作可能とする。
