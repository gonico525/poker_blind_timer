# ヘッダードロップダウン幅・起動時初期ブラインド表示の修正結果
...
| 修正2 | 起動時初期ブラインド・ネクストレベル表示の修正 | 完了 |
| 検証  | ビルド・テスト                                 | 完了 |
...
`MainLayout` で初期マウント時に `loadStructure(currentStructureId)` を呼び出し、`TournamentContext` にブラインドデータを注入する。`useRef` フラグで初回実行のみに制限し、明示的なストラクチャー変更時の重複実行を防ぐ。
...
これにより起動直後から `blindLevels` に実データが入り、ブラインド情報とネクストレベルが即座に表示される。
